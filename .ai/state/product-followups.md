...
| PP-2026-07-28-004 | Clean meeting mix (no doubled remote / broken transcript) | chat 2026-07-28 | Product bug / initiative | Major | Dual-path QA **passed** 2026-07-29 (speakers mic-only; BT + aux headphones; mid-record speakers→BT). Close on dual-path commit/ship; Ideal P0 met for tested routes. | Critical | product-analyst / roadmap-planner → implementer | Close when `recording-mix-dual-path` ships | open |
| PP-2026-07-28-005 | Speakers: fixed-delay remote double remains (bleed) | `2026-07-28-recording-mix-sync` | Supervisor QA | Product / initiative | **Cured by dual-path mic-only on speakers** (QA 2026-07-29 pass). Close on dual-path ship. | High | product-analyst / roadmap-planner | Close when dual-path ships | open |
| PP-2026-07-28-006 | Speakers overlap: user voice hard to hear under remote | `2026-07-28-recording-mix-sync` | Supervisor QA | Product / listening | Dual-path speakers QA passed (incl. modes exercised 2026-07-29). Re-open only if overlap still buried on mic-only. | Medium | product-analyst / roadmap-planner | Only if PO reports C still bad after dual-path | open |
| PP-2026-07-28-007 | Headphones: mic quiet vs system in mix | `2026-07-28-recording-mix-sync` | Supervisor QA | Product / listening | Dual-path headphones QA **passed** (BT + aux, 2026-07-29) with level-match. Close on ship unless PO still hears bury. | Medium | product-analyst / roadmap-planner | Close when dual-path ships | open |
| PP-2026-07-29-001 | Dual-path Record finalize: speakers→mic-only; headphones→mic+system with mic level match | `2026-07-28-recording-mix-aec-spike` | Supervisor QA / PO proposal | Product / Ideal (initiative Phase 3′) | **QA passed 2026-07-29** (4 modes). Close on commit/ship of `recording-mix-dual-path`. | Critical | product-analyst / roadmap-planner | Close when dual-path ships | open |
...
| PP-2026-07-29-004 | Clarify / optionally prefer headset mic when a headset is default output | `2026-07-29-recording-mix-dual-path` | Supervisor QA | Product question | Today Record uses macOS **default input** (`AVCaptureDevice.default`). Headset mic is used only if macOS made it the default mic — not guaranteed by headphone output alone. Tied to PP-003. | Medium | product-analyst / roadmap-planner | With PP-003 or dedicated mic-selection slice | open |
